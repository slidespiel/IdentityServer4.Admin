    ...
    public class IdentityResourceRepositoryTests
    {
        ...
        public async Task AddIdentityResourceAsync()
        {
            using (var context = new IdentityServerConfigurationDbContext(_dbContextOptions, _storeOptions))
            {
                ...
                newIdentityResource.Should().Be(identityResource, options => options.Excluding(o => o.Id));
            }
        }
        ...
        public async Task GetIdentityResourceAsync()
        {
            using (var context = new IdentityServerConfigurationDbContext(_dbContextOptions, _storeOptions))
            {
                ...
                //Assert new identity resource
                newIdentityResource.Should().Be(identityResource, options => options.Excluding(o => o.Id).Excluding(o => o.UserClaims));

                newIdentityResource.UserClaims.Should().Be(identityResource.UserClaims,
        ...
        public async Task UpdateIdentityResourceAsync()
        {
            using (var context = new IdentityServerConfigurationDbContext(_dbContextOptions, _storeOptions))
            {
                ...
                updatedIdentityResource.Should().Be(updatedIdentityResourceEntity);
            }
        }
		...
		public async Task AddIdentityResourcePropertyAsync()
		{
			using (var context = new IdentityServerConfigurationDbContext(_dbContextOptions, _storeOptions))
			{
				...
				//Assert new identity resource
				resource.Should().Be(identityResource, options => options.Excluding(o => o.Id)
                    .Excluding(o => o.UserClaims));

                resource.UserClaims.Should().Be(identityResource.UserClaims,
                    option => option.Excluding(x => x.SelectedMemberPath.EndsWith("Id"))
                        .Excluding(x => x.SelectedMemberPath.EndsWith("IdentityResource")));
				...
				resourceProperty.Should().Be(identityResourceProperty,
		...
		public async Task DeleteIdentityResourcePropertyAsync()
		{
			using (var context = new IdentityServerConfigurationDbContext(_dbContextOptions, _storeOptions))
			{
				...
				//Assert new identity resource
				resource.Should().Be(identityResource, options => options.Excluding(o => o.Id).Excluding(o => o.UserClaims));
				...
				//Assert
				property.Should().Be(identityResourceProperty,
					options => options.Excluding(o => o.Id).Excluding(x => x.IdentityResource));
